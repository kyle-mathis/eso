			...
			if saveFile.data[ map ][ pinTypeId ] == nil then
				...
				for nodeIndex, node in pairs( data[ pinTypeId ] ) do
					...
					if success then -- check if something went wrong while deserializing the node
						...
						isValid = true
						if Harvest.GetMaxTimeDifference() > 0 and (timestamp - stamp) > Harvest.GetMaxTimeDifference() then
							isValid = false
						end
...
function Data:LoadToCache(pinTypeId, cache)
	...
	for nodeIndex, node in pairs( serializedNodes ) do
		...
		if not success then
			...
			if Harvest.GetMaxTimeDifference() > 0 and currentTimestamp - timestamp > Harvest.GetMaxTimeDifference() then
				valid = false
			end
			--[[
