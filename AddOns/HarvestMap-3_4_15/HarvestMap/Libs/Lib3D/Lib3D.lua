...
- origin update doesn't work when teleporting because of respawning
- measures for delves, public dungeons and other smaller subzones can be off, resulting in translated pins
--]]
...
local function OnPlayerActivated()
	...
	local worldX, worldY, worldZ = lib:GetCameraCurrentWorldCoords()
	d("world camera coords", worldX, worldY, worldZ)
	local closeOrigin = (worldX * worldX + worldZ * worldZ < MAXIMUM_CAMERA_DISTANCE + 1)
...
function lib:GetGlobalToWorldFactor(zoneId)
	return lib.SPECIAL_GLOBAL_TO_WORLD_FACTORS[zoneId]
end
