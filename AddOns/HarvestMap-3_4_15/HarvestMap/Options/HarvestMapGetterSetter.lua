...
function Harvest.GetMinGameVersion()
	return Harvest.savedVars["settings"].minGameVersion
end
...
function Harvest.GetDisplayedMinGameVersion()
	...
	local versionNumber = Harvest.savedVars["settings"].minGameVersion
...
function Harvest.SetFarmingInterfaceHidden( value )
	...
	CallbackManager:FireCallbacks(Events.SETTING_CHANGED, "farmingInterfaceHidden", value)
	if value then -- TODO via callback
...
function Harvest.ApplyTimeDifference()
	...
	if versionString then
		...
		Harvest.savedVars["settings"].minGameVersion = versionNumber
	end
