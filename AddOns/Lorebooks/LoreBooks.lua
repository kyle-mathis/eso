...
local function EideticValidEntry(categoryIndex, collectionIndex, bookIndex, bookName)
	...
	elseif categoryIndex == 2 then
		...
		or collectionIndex == 28 -- The Unknown collection which regroup craft motifs
		or collectionIndex == 29 -- Akaviri
		or collectionIndex == 30 -- Yokudan
		or collectionIndex == 31 -- Glass
		or collectionIndex == 32 -- Mercenary
		or collectionIndex == 33 -- Xivkyn
		or collectionIndex == 34 -- Anc Orc
		
		-- 35 = Orsinium Archives
		-- 36 = Hew's Bane Bookshelf
		-- 37 = Gold Coast Tomes
		
		or collectionIndex == 38 -- Outlaw
		or collectionIndex == 39 -- Trinimac
		or collectionIndex == 40 -- Malacath
		or collectionIndex == 41 -- Aldmeri
		or collectionIndex == 42 -- Daggerfall
		or collectionIndex == 43 -- Ebonheart
		...
		-- 48 = Library of Incunabula
		
		or collectionIndex == 49 -- Draugr
		or collectionIndex == 50 -- Minotaur
		or collectionIndex == 51 -- Order of the Hour
		or collectionIndex == 52 -- Celestial
		or collectionIndex == 53 -- Hallowjack (hidden)
		or collectionIndex == 54 -- Dark Brotherhood
		or collectionIndex == 55 -- Grim Harlequin (hidden)
		or collectionIndex == 56 -- Skinchanger
...
local function BuildDataToShare(bookName, medium)
	...
	if bookName and bookName ~= "" then
		...
		if loreBoooksLibrary[bookName] then -- Books in Eidetic. Few are not.
			...
			if EideticValidEntry(categoryIndex, collectionIndex, bookIndex, bookName) then
				...
				dataToShare = dataToShare .. "@" .. bookRef .. ";" .. medium
				
				if loreBoooksLibrary[bookName].gotDuplicates then
					dataToShare = dataToShare .. "@1" -- categoryIndex, collectionIndex & bookIndex are false. CollabMiner will determine with medium & bookId which book user really read.
				else
					local bookLink = GetLoreBookLink(categoryIndex, collectionIndex, bookIndex)
					local bookId = select(4, ZO_LinkHandler_ParseLink(bookLink))
					dataToShare = dataToShare .. ";" .. UnsignedBase62(tonumber(bookId))
				end
...
local function OnShowBook(_, bookName, _, medium)
	local dataToShare = BuildDataToShare(bookName, medium)
	if dataToShare then
...
local function ShowCongrats(newBook)
	...
	local congrats = {
		...
		[3087] = "/esoui/art/icons/ability_mage_064.dds", -- Max SoH & 1T (was 3073 for both but raised at 3087 since 2.6.7)
	}
	...
	if newBook then
		...
		if congrats[totalCurrentlyCollected] then
			if (totalCurrentlyCollected == 3087 and GetAPIVersion() == SUPPORTED_API) then
				CongratsStuff(totalCurrentlyCollected)
			elseif totalCurrentlyCollected ~= 3087 then
				CongratsStuff(totalCurrentlyCollected)
			end
		end
...
function LoreBooks_RefreshEideticOptions(self)
	
	local checkboxs = {"BelongToQuest", "InDungeon"}
	
	local somethingChecked
	for checboxIndex, checkboxName in ipairs(checkboxs) do
		local control = GetControl(LOREBOOKS_HELP.eideticOptions, checkboxName)
		local isChecked = ZO_CheckButton_IsChecked(control)
		if isChecked then
	...
	local button = GetControl(LOREBOOKS_HELP.eideticOptions, "SendData")
...
function Lorebook_ClickHelpBook(control, button)
	if button == MOUSE_BUTTON_INDEX_LEFT then
		--if control.data.categoryIndex == 2 then
...
local function OnHelpSearchTextChanged(self)
	...
	if string.len(search) >= 5 or string.len(search) == 0 then
		if string.len(search) == 0 then
			...
			LOREBOOKS_HELP.eideticOptions:SetHidden(true)
		end
...
function LOREBOOKS_HELP:Initialize(control)
	...
	LOREBOOKS_HELP.eideticOptions = control:GetNamedChild("EideticOptions")
...
function LoreBooks_SendReport(mode)
	...
	elseif mode == 2 then
		data = string.format("%s;%s;%s;q=%s;qn=%s;d=%s", ADDON_VERSION, mode, LOREBOOKS_HELP.reference:GetText(), LOREBOOKS_HELP.eideticOptions:GetNamedChild("BelongToQuest"):GetState(), LOREBOOKS_HELP.eideticOptions:GetNamedChild("QuestBox"):GetText(), LOREBOOKS_HELP.eideticOptions:GetNamedChild("InDungeon"):GetState())
	end
...
local function ShowReportForShalidor(collectionIndex, bookIndex, x, y, zone, subzone, mapIndex)
	...
	if GetAPIVersion() == SUPPORTED_API then
		...
		local eideticEligible = GetControl(LOREBOOKS_HELP.control, "EligibleForEideticReport")
		eideticEligible:SetHidden(true)
...
local function ShowReportForEidetic(collectionIndex, bookIndex, x, y)

	local function IsEideticEligibleForReport(collectionIndex, bookIndex, x, y)
		
		--User use old ref, but we need to check the new ref
		local bookData = LoreBooks_GetNewEideticData(2, collectionIndex, bookIndex)
		if bookData and bookData.e then
			for entryIndex, entryData in ipairs(bookData.e) do
				if x == entryData.x and y == entryData.y then
					return true, entryIndex, bookData
				end
			end
		end
	...
	local reference = string.format("{ %.5f, %.5f, %d, %d }", x, y, collectionIndex, bookIndex)
	...
	if GetAPIVersion() == SUPPORTED_API then
		...
		local eideticEligible = GetControl(LOREBOOKS_HELP.control, "EligibleForEideticReport")
		eideticEligible:SetHidden(false)
		
		--[[
		if totalCurrentlyCollected > THREESHOLD_EIDETIC_REPORT then
			
			local bookEligibleForReport, entryIndex, bookData = IsEideticEligibleForReport(collectionIndex, bookIndex, x, y)
			
			local button = GetControl(LOREBOOKS_HELP.eideticOptions, "SendData")
			button:SetState(BSTATE_DISABLED)
			
			if bookEligibleForReport then
				eideticEligible:SetText(GetString(LBOOKS_RE_ELIGIBLE))
				
				local isDRef = "MainMap"
				if bookData.e[entryIndex].d then --InDungeon
					isDRef = "Dungeon=" .. bookData.e[entryIndex].z
				end
				
				local reference = string.format("{ %.5f, %.5f, %d, %d, %s }", bookData.e[entryIndex].x, bookData.e[entryIndex].y, collectionIndex, bookIndex, isDRef)
				LOREBOOKS_HELP.reference:SetText(reference)
				LOREBOOKS_HELP.eideticOptions:SetHidden(false)
			else
				eideticEligible:SetText(GetString(LBOOKS_RE_NOT_ELIGIBLE))
				LOREBOOKS_HELP.eideticOptions:SetHidden(true)
			end
...
local function ReportBook(categoryIndex, collectionIndex, bookIndex, x, y, zone, subzone, mapIndex)
	...
	elseif categoryIndex == 2 then
		ShowReportForEidetic(collectionIndex, bookIndex, x, y)
	end
		...
		for mapIndex = 1, GetNumMaps() do
			...
			if eideticBooks then
				...
				for _, bookData in ipairs(eideticBooks) do
					local _, _, known = GetLoreBookInfo(2, bookData.c, bookData.b)
				...
				if missingInMap > 0 then
					...
					for index, data in ipairs(eideticData[mapIndex]) do
						local bookName = GetLoreBookInfo(2, data.c, data.b)
...
local function BuildEideticReportPerCollection(lastObject)
	...
	if totalCurrentlyCollected >= THRESHOLD_EIDETIC then
		...
		local categoryName, numCollections = GetLoreCategoryInfo(2) -- Only Eidetic
		
		for collectionIndex = 1, numCollections do
			
			eideticData[collectionIndex] = {}
			
			local collectionName, _, _, totalBooksInCollection, hidden = GetLoreCollectionInfo(2, collectionIndex)
			
			if not hidden then
				
				for bookIndex = 1, totalBooksInCollection do
					local bookName, _, known = GetLoreBookInfo(2, collectionIndex, bookIndex)
				...
				if missingInCollection > 0 then
					...
					for bookIndex, bookName in pairs(eideticData[collectionIndex]) do
						...
						local bookLocation = ""
						if EideticValidEntry(2, collectionIndex, bookIndex) then
							local bookData = LoreBooks_GetNewEideticData(2, collectionIndex, bookIndex)
							if bookData then
								if bookData.r then
									bookLocation = "[B] "
...
local function BuildCategoryList(self)
	...
	for i, categoryData in ipairs(categories) do
		...
		for _, collectionData in ipairs(collections) do
...
local function OnRowMouseUp(control, button)
	if button == MOUSE_BUTTON_INDEX_RIGHT then
		...
		elseif control.categoryIndex == 2 and EideticValidEntry(control.categoryIndex, control.collectionIndex, control.bookIndex) then
...
local function OnMouseEnter(self, categoryIndex, collectionIndex, bookIndex)
	...
	-- No 1 for now.
	if categoryIndex == 2 and EideticValidEntry(categoryIndex, collectionIndex, bookIndex) and not mapIsShowing then
...
local function CreatePins()
	...
	LMP:AddPinFilter(PINS_EIDETIC, GetLoreCategoryInfo(2), nil, db.filters)
	...
	if totalCurrentlyCollected >= THREESHOLD_EIDETIC then
		...
		LMP:AddPinFilter(PINS_EIDETIC_COLLECTED, zo_strformat(LBOOKS_FILTER_EICOLLECTED, GetLoreCategoryInfo(2)), nil, db.filters)
	end
		...
		[1] = {
			...
			callback = function(pin)
				if IsPlayerOnCurrentMap() then
					local zone, subzone = LMP:GetZoneAndSubzone()
					local mapIndex = GetCurrentMapIndex()
					ReportBook(1, pin.m_PinTag[3], pin.m_PinTag[4], pin.m_PinTag[1], pin.m_PinTag[2], zone, subzone, mapIndex)
				end
			end,
		}
	})
	...
	LMP:SetClickHandlers(PINS_EIDETIC, {
		[1] = {
			name = function(pin) return zo_strformat(LBOOKS_SET_WAYPOINT, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) end,
			show = function(pin) return not select(3, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) end,
	...
	{
		[1] = {
			name = function(pin) return zo_strformat(LBOOKS_REPORT_BOOK, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) end,
			show = function(pin) return not select(3, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) end,
			duplicates = function(pin1, pin2) return (pin1.m_PinTag.b == pin2.m_PinTag.c and pin1.m_PinTag.b == pin2.m_PinTag.b) end,
			callback = function(pin)
				if IsPlayerOnCurrentMap() then
					ReportBook(2, pin.m_PinTag.c, pin.m_PinTag.b, pin.m_PinTag.x, pin.m_PinTag.y)
				end
			end,
		}
	})
	LMP:SetClickHandlers(PINS_EIDETIC_COLLECTED, {
		[1] = {
			name = function(pin) return zo_strformat(LBOOKS_SET_WAYPOINT, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) end,
			show = function(pin) return select(3, GetLoreBookInfo(2, pin.m_PinTag.c, pin.m_PinTag.b)) == true end,
			duplicates = function(pin1, pin2) return (pin1.m_PinTag.b == pin2.m_PinTag.c and pin1.m_PinTag.b == pin2.m_PinTag.b) end,
			callback = function(pin) PingMap(MAP_PIN_TYPE_PLAYER_WAYPOINT, MAP_TYPE_LOCATION_CENTERED, pin.normalizedX, pin.normalizedY) end,
		}
	},
